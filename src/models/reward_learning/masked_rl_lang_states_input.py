...
class MaskedRL:
    def __init__(self, params, featurizer, demos, all_trajs, 
                 ...
                 language_ambiguity=None, llm_disambiguation=False,
                 **kwargs):
        """
        ...
        self.demo_language_instructions = theta_to_language(demo_thetas, self.language_ambiguity, llm_disambiguation=self.llm_disambiguation)
        self.train_language_instructions = theta_to_language(train_thetas, self.language_ambiguity, llm_disambiguation=self.llm_disambiguation)
        self.demo_state_masks = theta_to_state_mask(demo_thetas, state_dim=self.state_dim)
        self.finetune_demo_language_instructions = theta_to_language(finetune_demo_thetas, self.language_ambiguity, llm_disambiguation=self.llm_disambiguation)

        # choose one from demo_indices
        self.demo_state_masks = theta_to_state_mask(demo_thetas, state_dim=self.state_dim)
        self.train_state_masks = theta_to_state_mask(train_thetas, state_dim=self.state_dim)
        self.finetune_demo_state_masks = theta_to_state_mask(finetune_demo_thetas, state_dim=self.state_dim)
        ...
        self.masked_loss_weight = params.get("masked_loss_weight", 1.0)
        self.masked_loss_noise = params.get("masked_loss_noise", 0.1)
    ...
    def evaluate(self, human_win_rates, num_samples=100, return_rewards=False):
        ...
        for info in human_win_rates:
            ...
            inst = theta_to_language([theta], self.language_ambiguity, llm_disambiguation=self.llm_disambiguation)[0]
            with torch.no_grad():
    ...
    def train(self, iterations, save_model=False, save_loss=False):
        ...
        for epoch in p:
            ...
            for i in range(0, num_demos, self.batch_size):
                ...
                # Create a mask for state features based on demo_state_masks.
                # Assume self.demo_state_masks[demo_indices] has shape (B, state_dim).
                mask = self.demo_state_masks[demo_indices].unsqueeze(1).repeat(1, demo_batch.shape[1], 1).to(torch.float64)
                mask = mask.repeat(num_repeat, 1, 1)  # shape: (B*num_repeat, T, state_dim)

                # Perturb the demo batch: only perturb state features where mask==0.
                perturbed_demo = repeated_demo + random_noise * (1 - mask)
                # Compute the trajectory cost on the perturbed demos.
                perturbed_cost = self.calc_traj_cost_batch(perturbed_demo, repeated_lang).unsqueeze(1)  # shape: (B*num_repeat, 1)

                # Compute the original cost for the demo batch.
                original_cost = self.calc_traj_cost_batch(demo_batch, demo_lang_emb_batch)  # shape: (B,)
                # Repeat original cost to match perturbed batch dimensions.
                original_cost_repeated = original_cost.repeat(num_repeat, 1).view(-1, 1)

                # Compute the masked loss as the mean absolute difference.
                masked_loss = torch.mean(torch.abs(original_cost_repeated - perturbed_cost))
                
                # Diagnostics BEFORE backward
                with torch.no_grad():
                    demo_cost_mean = cost_demos.mean().item()
                    train_cost_mean = cost_train.mean().item()
                    demo_cost_std = cost_demos.std().item()
                    train_cost_std = cost_train.std().item()

                loss = maxent_loss + masked_loss * self.masked_loss_weight
                loss.backward()
            ...
            if self.wandb:
                ...
                wandb.log({
                    ...
                    "train/maxent_loss": epoch_maxent_loss,
                    "train/masked_loss": epoch_masked_loss,
    ...
    def finetune(self, finetune_iterations=30, start_iteration=100, save_model=True):
        ...
        for epoch in p:
            ...
            for i in range(0, num_finetune_demos, self.batch_size):
                ...
                # Compute masked loss
                num_repeat = 10
                repeated_demo = demo_batch.repeat(num_repeat, 1, 1)
                repeated_lang = demo_lang_emb_batch.repeat(num_repeat, 1)
                random_noise = torch.randn_like(repeated_demo) * self.masked_loss_noise
                
                mask = self.finetune_demo_state_masks[demo_indices].unsqueeze(1).repeat(1, demo_batch.shape[1], 1).to(torch.float64)
                mask = mask.repeat(num_repeat, 1, 1)
                
                perturbed_demo = repeated_demo + random_noise * (1 - mask)
                perturbed_cost = self.calc_traj_cost_batch(perturbed_demo, repeated_lang).unsqueeze(1)
                
                original_cost = self.calc_traj_cost_batch(demo_batch, demo_lang_emb_batch)
                original_cost_repeated = original_cost.repeat(num_repeat, 1).view(-1, 1)
            ...
            epoch_maxent_loss /= num_finetune_demos
            epoch_masked_loss /= num_finetune_demos
            p.set_description('Finetune Epoch %d loss: %.8f' % (epoch + 1, epoch_loss))
            ...
            if self.wandb:
                ...
                wandb.log({
                    ...
                    "finetune/maxent_loss": epoch_maxent_loss,
                    "finetune/masked_loss": epoch_masked_loss,
