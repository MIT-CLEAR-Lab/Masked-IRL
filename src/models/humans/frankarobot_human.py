...
class FrankarobotHuman(BaseHuman):
    ...
    def __str__(self):
        return "FrankarobotHuman " + str(self.theta)
    ...
    def calc_eef_object_states(self, traj, object_centers=None, state_dim=9):
        ...
        #     table_height = p.getBasePositionAndOrientation(self.env.objectID["stand"])[0][2]
        #     human_xy = p.getBasePositionAndOrientation(self.env.objectID["human"])[0][0:2]
        # else:
        #     table_height = object_centers["TABLE_CENTER"][2]
        #     human_xy = object_centers["HUMAN_CENTER"][:2]
        #     # laptop_pos = object_centers["LAPTOP_CENTER"][:2]
        # # laptop_pos = p.getBasePositionAndOrientation(self.env.objectID["laptop"])[0][0:3]
        
        for waypoint in traj:
            if state_dim == 9:
                # Extract end-effector position and rotation from the waypoint.
                # Adjust indices based on your trajectory data format.
                eef_pos = np.array(waypoint[112:115])    # 3 DoF position
                # eef_rot = np.array(waypoint[79:88])      # 9 DoF rotation (quaternion)
                eef_rot = np.array(waypoint[85:86]) # 1 DoF rotation (z-axis)
                laptop_pos = np.array(waypoint[118:120])  # 2 DoF laptop 
                # added
                human_xy = np.array(waypoint[115:117])  # 2 DoF human position
                # table_height = waypoint[121]  # 1 DoF table height
                table_height = -0.65
                ...
                eef_rot = np.array(waypoint[79:88])      # 9 DoF rotation matrix
                laptop_pos = np.array(waypoint[118:121])  # 3 DoF laptop position
                human_xy = np.array(waypoint[115:118])  # 3 DoF human position
                table_height = -0.65 #waypoint[121]  # 1 DoF table height
                # Concatenate into a single state vector.
                state = np.concatenate((eef_pos, eef_rot, 
                                        np.array(human_xy), 
                                        np.array(laptop_pos),
                ...
                eef_pos = np.array(waypoint[112:115])    # 3 DoF position
                # use 9-dim rotation matrix 
                eef_rot = np.array(waypoint[85:86])      # 9 DoF rotation matrix
                laptop_pos = np.array(waypoint[118:121])  # 3 DoF laptop position
                human_xy = np.array(waypoint[115:118])  # 3 DoF human position
                table_height = -0.65 #waypoint[121]  # 1 DoF table height
                # Concatenate into a single state vector.
    ...
    def table_features(self, traj, object_centers=None):
        ...
        for waypt in traj:
            ...
            if object_centers is None:
                stand_z = p.getBasePositionAndOrientation(self.env.objectID["stand"])[0][2] # support height
            else:
                stand_z = p.getBasePositionAndOrientation(self.env.objectID["stand"])[0][2] + object_centers["TABLE_CENTER"][2]
            feat_val += waypt[114] - stand_z
        return feat_val
    ...
    def laptop_features(self, traj, object_centers=None):
        ...
        for waypt in traj:
            ...
            if object_centers is None:
                dist = np.linalg.norm(waypt[112:114] - waypt[118:120]) - 0.8
            else:
                dist = np.linalg.norm(waypt[112:114] - object_centers["LAPTOP_CENTER"][:2]) - 0.8
            feat_val += -((dist < 0) * dist)
        return feat_val
    ...
    def proxemics_features(self, traj, object_centers=None):
        ...
        feat_val = np.zeros(1)
        for waypt in traj:
            # waypt2 = np.hstack((waypt[:7], np.array([0, 0, 0])))
            # move_robot(self.env.objectID["robot"], joint_poses=waypt2)
            # coords = robot_coords(self.env.objectID["robot"])
            # EE_coord_xy = coords[6][0:2]
            # posH, _ = p.getBasePositionAndOrientation(self.env.objectID["human"])
            # human_xy = list(posH[0:2])

            EE_coord_xy = copy.deepcopy(waypt[112:114])
            if object_centers is None:
