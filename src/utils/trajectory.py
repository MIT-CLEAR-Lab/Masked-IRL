...
class Trajectory(object):
    ...
    """
    def __init__(self, waypts, waypts_time, robot_type):
        self.waypts = waypts
    ...
    def downsample(self, num_waypts):
        ...
        for index in range(num_waypts):
            ...
            waypts_time[index] = t
            waypts[index,:] = self.interpolate(t).reshape((1, 10))
    ...
    def interpolate(self, t):
        ...
        waypt = np.array(waypt).reshape((10, 1))
        return waypt
    ...
    def deform(self, u_h, t, alpha, n):
        ...
        if self.robot_type == "jaco":
            ...
            deform_dim = 9 # 0-6, 9-10
        else:
            raise ValueError("Invalid robot type. Please specify 'jaco' or 'franka'.")
        waypts_deform = copy.deepcopy(self.waypts)
        # gamma = np.zeros((n, 10))
        gamma = np.zeros((n, deform_dim))
        deform_waypt_idx = int((t - self.waypts_time[0]) / self.timestep)
        if (deform_waypt_idx + n) > self.num_waypts:
            print("Deforming too close to end. Returning same trajectory")
            return Trajectory(waypts_deform, self.waypts_time, self.robot_type)

        # for joint in range(10):
        for joint in range(deform_dim):
